...
const RuleSchema = new _mongoose2.default.Schema({
  description: {
    type: String,
...
RuleSchema.statics = {
  aggregateScore: async function (userId) {
    ...
    try {
      ...
      return testedRules.reduce((accumulator, currentValue) => accumulator + currentValue.points, 0);
    } catch (ex) {
      log.error(ex);
      throw ex;
    }
  ...
  _getVariance: function (binary1, binary2) {
    ...
    try {
      if (binary1.length !== binary2.length) {
        throw (0, _boom.badRequest)("Binary lengths do not match");
      }
...
RuleSchema.methods = {
  ...
  getScoreValue: function (testBinary) {
    try {
      return testBinary === this.ruleBinary ? { match: true, points: this.ruleBinary.split("1").length - 1 } : { match: false, points: 0 };
    } catch (ex) {
      log.error(e);
      throw e;
    }
  ...
  __testedDuplicatesExist: async function () {
    try {
      ...
      const duplicates = await _mongoose2.default.model("Rules").find({
        ruleBinary: this.ruleBinary,
        user: this.user,
        remainingAttempts: 0
      });
...
RuleSchema.path("ruleBinary").validate({
  validator: function (ruleBinary) {
    return new Promise(async (resolve, _) => {
      try {
        const script = await _2.ScriptsModel.findOne({ _id: this.script });
...
exports.default = _mongoose2.default.model("Rules", RuleSchema);
