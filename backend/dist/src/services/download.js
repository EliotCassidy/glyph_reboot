...
exports.DownloadService = undefined;
...
class DownloadService {
  ...
  async exportRulesForScript({ csvStream, scriptId }) {
    ...
    try {
      ...
      if (!unicode) throw (0, _boom.notFound)('Could not find unicode characters for this script');

      for (let x = 0; x < unicode.length; x++) {
        const character = unicode[x];
        const row = { 'Character': character };

        for (let _ref of rules) {
          let { ruleBinary, _id } = _ref;
...
exports.default = options => new DownloadService(options);
